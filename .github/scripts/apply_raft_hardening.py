...
# ---------------------------------------------------------------------------
# Pure Raft engine: validate the protocol boundary and never overstate what an
# individual replication RPC actually proved.
# ---------------------------------------------------------------------------
replace_once(
    "src/raft.rs",
...
# Live configuration must fail at startup rather than relying on the pure
# engine's defensive normalization.
replace_once(
    "src/raft_driver.rs",
    """    ) -> io::Result<Arc<Self>> {
...
# ---------------------------------------------------------------------------
# WAL recovery: only an unterminated final fragment is torn; every complete
# malformed record and every impossible hard-state/log relationship fails closed.
# ---------------------------------------------------------------------------
replace_once(
    "src/raft_store.rs",
...
# ---------------------------------------------------------------------------
# Regression tests for the protocol bugs and recovery invariants.
# ---------------------------------------------------------------------------
insert_before_once(
    "src/raft.rs",
    ...
    fn heartbeat_acknowledges_only_the_prefix_this_request_proved() {
        ...
        assert_eq!(follower.last_index(), 2, "the unproved suffix remains local");
    }
...
insert_before_once(
    ...
    fn descending_log_terms_are_rejected() {
        ...
        store
            ...
            .unwrap();
        store
            .write_log(0, 0, &[entry(1, 3), entry(2, 2)])
            .unwrap();
...
Path("docs/raft-safety-boundaries.md").write_text(
